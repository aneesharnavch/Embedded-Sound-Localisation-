...
\textbf{Results.} Confirmatory numerical refinements change checked estimates by less than $0.021^\circ$. At 10 dB direct-referenced SNR, direct-path RMSE increases from $0.929^\circ$ to $2.279^\circ$ after arrival rounding. In the high-reverberation subset, first-frame RMSE is $4.59^\circ$ at source onset versus $40.42^\circ$ with populated prehistory. Averaging 128 frames reduces held-out RMSE from $1.60^\circ$ to $0.84^\circ$ in the low subset and from $39.76^\circ$ to $10.04^\circ$ in the high subset. A geometry-only timing bound accounts for shared-microphone dependence.
...
\end{abstract}
\vspace{3mm}
\noindent\textbf{Keywords:} acoustic localization; GCC-PHAT; image-source method; numerical verification; source history; uncertainty
\vspace{5mm}]
\linenumbers
...
\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{fig1_geometry_controls.pdf}
...
At full selected image coverage, the most demanding independent room check compares approximately 2.15 million paths per microphone. Relative spectral discrepancy is $3.20\times10^{-5}$ and angle discrepancy is $0.00181^\circ$. Coarse image coverage changed one high-setting estimate by more than $100^\circ$ before convergence. A coarse fractional grid also missed the spectral target; both findings are documented in the numerical notes. They motivated refinement, not relaxation of the tolerance.

On the confirmatory secondary subset, increasing propagation oversampling from 16 to 32 changes individual estimates by at most $0.02048^\circ$; image extent 48 to 56 changes them by $0.00424^\circ$; GCC factor 8 to 16 changes them by $0.01399^\circ$. SRP grid refinement changes estimates by at most $0.00305^\circ$. No checked stochastic frame exceeds $0.05^\circ$ (Fig.~\ref{fig:numerics}). These selected checks do not constitute a worst-case theorem for every possible stochastic input.

\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{fig2_numerical_verification.pdf}
...
At 10 dB, the corrected core RMSE is $1.616^\circ$ [95\% interval 1.597, 1.635] for nominal 0.15 s, $12.112^\circ$ [11.253, 12.956] for 0.30 s, and $37.901^\circ$ [37.078, 38.759] for 0.60 s. The proportions above $5^\circ$ are 0.77\%, 27.40\%, and 62.03\%. The corresponding 20-dB RMSEs are 1.329, 11.495, and $37.328^\circ$: reducing additive noise does not eliminate reflected-field errors in this model.

Figure~\ref{fig:core} preserves differences across rooms. At 10 dB, individual scene RMSE ranges from 0.84 to $2.86^\circ$ in the lowest setting, 2.03 to $47.53^\circ$ in the middle setting, and 4.47 to $83.48^\circ$ in the highest. These ranges are much wider than uncertainty intervals on pooled conditional means. Direction, placement, and room geometry are therefore material parts of the reported result.

\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{fig3_core_rooms.pdf}
...
Across the 72 attribution scenes at 10 dB, steady direct-path RMSE is $0.929^\circ$ [0.920, 0.939] with fractional arrivals and $2.279^\circ$ [2.266, 2.293] with rounding. The paired increase in mean squared error is 4.330 deg$^2$ [4.272, 4.388]. This establishes a rounding effect after waveform processing, separately from the analytical geometry diagnostic.

With reflections, steady RMSE is $28.389^\circ$ for fractional arrivals and $28.729^\circ$ after rounding. The paired squared-error change is 19.409 deg$^2$, but its interval [$-2.396$, 42.332] spans zero. Thus, the clear direct-path rounding penalty does not justify a universal same-sign penalty in a coherent reflected field. Rounding changes interference and can alter which peak is selected.

The source-history contrast is strongest immediately after startup (Fig.~\ref{fig:paired}). In the high subset, the first-frame RMSE is $4.591^\circ$ at onset versus $40.415^\circ$ with populated prehistory. By the last frame of the 1.365-s short record, both reach $38.746^\circ$ for the matched realizations. For fractional reflected signals pooled across all 72 scenes and 32 frames, onset reduces mean squared error by 56.011 deg$^2$ [36.349, 77.029], expressed as a reduction; onset-minus-steady is negative. This describes transient observation, not an improvement to the estimator.

The factorial onset-by-reflection interaction is $-52.601$ deg$^2$ [$-70.892$, $-33.762$]. The corresponding rounding-by-reflection interval spans zero. Interactions and broad error tails make an independent additive error budget inappropriate. The separate exploratory per-frame-reset diagnostic gives much lower reflected RMSE than continuous records, but it uses pilot directions and is not substituted for the confirmatory time trajectory.

\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{fig4_paired_attribution.pdf}
\caption{Paired attribution at 10 dB. (a) Eight combinations: F/Q, fractional/rounded; S/O, steady/onset; D/R, direct/reflected. Low and high subsets each contain 36 scenes; their placement and absorption settings differ. Error bars are record-bootstrap intervals. (b) Matched fractional reflected recordings in the high subset. The first onset frame contains little established reflected energy; later frames approach the steady condition. Time is measured from the beginning of the analyzed recording, and markers show frame midpoints.}
\label{fig:paired}
\end{figure*}

\subsection{Independent-record averaging}
Held-out results confirm a reduction in dispersion with longer averaging, without a common residual accuracy across scenes (Fig.~\ref{fig:averaging}). In the low subset, RMSE changes from $1.598^\circ$ [1.585, 1.612] for one frame to $0.842^\circ$ [0.824, 0.861] for 128 frames. In the high subset, it changes from $39.755^\circ$ [39.117, 40.415] to $10.041^\circ$ [9.298, 10.700]. At 128 frames, 45\% of high-subset block estimates still exceed $5^\circ$.

Forty-eight of 72 scenes pass the training concentration rule. Their held-out 128-frame RMSE is $2.034^\circ$ [1.934, 2.133], compared with $2.002^\circ$ from the bias/covariance approximation and $2.028^\circ$ from the independent-frame approximation. This close aggregate agreement is compatible with the model on the selected set; the tiny difference between the two approximations does not establish a useful superiority of covariance correction. The empirical results for the excluded, less concentrated scenes remain in panel (a) and the supplement. A 5.46-s observation is finite: it does not establish an asymptotic or universal reverberation floor.

\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{fig5_heldout_averaging.pdf}
\caption{Averaging on independent held-out recordings. (a) Circular block means in all 72 scenes, separated into low and high subsets. (b) Only the 48 scenes passing the rule determined from separate training records. Predictions are model outputs, not confidence intervals. Shaded intervals resample whole held-out recordings within each fixed scene.}
\label{fig:averaging}
\end{figure*}
...
The secondary subset demonstrates the limits of extrapolation. GCC-PHAT RMSE is $22.682^\circ$ in its base condition, $23.609^\circ$ after rotation, $18.916^\circ$ with the unequal triangle, $21.114^\circ$ with unequal walls, and $42.496^\circ$ for the lower-band source. These variations use new random records and do not define a geometry optimization. The lower-band test also changes how much of the fixed estimator band contains useful source energy.

On the same base frames, SRP-PHAT has $29.909^\circ$ RMSE versus GCC-PHAT's $22.682^\circ$, but a lower $5^\circ$ exceedance rate, 28.70\% versus 31.09\%. The paired squared-error difference is 380.063 deg$^2$ [290.101, 469.786]. The disagreement between RMSE and threshold rate reflects tail behavior; neither scalar alone supports a universal ranking. The complete method and scene summaries are retained.

\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{fig6_precision_sensitivity.pdf}
