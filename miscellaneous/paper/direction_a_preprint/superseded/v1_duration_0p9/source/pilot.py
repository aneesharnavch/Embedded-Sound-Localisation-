...
def run():
    ...
    for room,(placement,rt),angle in product(range(3),((0,0),(1,2)),(37.37,217.37)):
        ...
        fine,_=build_rirs(replace(cfg,oversample=32));more,_=build_rirs(replace(cfg,image_extent=56))
        for record_id,snr in product(range(2),(10.,float('inf'))):
            record=inputs(cfg,d,32,record_id,snr,'pilot_v1')
            t=time.perf_counter()
            base_y=render_record(h,record,cfg.fs);base=estimate(base_y,xy)
            for label,fh,ec in [('propagation32',fine,EstimatorConfig()),('extent56',more,EstimatorConfig()),('correlation16',h,EstimatorConfig(interp=16))]:
    ...
    for label in ('propagation32','extent56','correlation16'):
