...
def paths(config):
    ...
    if not config.reflections:
        ...
        return
    # A path inside the radial cutoff must also satisfy each axial cutoff.
    # Since source/microphone coordinates lie inside a side of length L,
    # |n| <= c*T/(2*L)+1 safely contains both parity cases. This only skips
    # tiles that cannot contribute; retained paths keep their original order.
    limits=np.minimum(config.image_extent,np.ceil(C*config.path_duration_s/(2*np.asarray(config.dimensions))+1).astype(int))
