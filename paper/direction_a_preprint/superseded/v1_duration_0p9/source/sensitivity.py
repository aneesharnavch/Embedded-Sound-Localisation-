...
def run():
    ...
    for room,(pl,rt),aid in product(range(3),((0,0),(1,2)),(0,3,6,9)):
        ...
        fine=build_rirs(replace(base,oversample=32))[0];more=build_rirs(replace(base,image_extent=56))[0]
        ...
        for label,(cfg,band) in variations.items():
            ...
            for rid in range(5):
                ...
                record=inputs(cfg,d,32,rid,10.,'main_v1' if label=='base' else 'sensitivity_v1_'+label,band)
                ...
                if label=='base':
                    ...
                    refined={'propagation32':estimate(render_record(fine,record,cfg.fs),xy),
                        'extent56':estimate(render_record(more,record,cfg.fs),xy),
