...
def check(job):
    ...
    short=np.load(OUT/'cache/rirs'/f'{tag}.npz')['fractional_room']
    longer=replace(cfg,image_extent=76,path_duration_s=1.2)
    ...
    deltas=[];mse_changes=[]
    for rid in range(5):
        record=inputs(cfg,d,32,rid,10.,'joint_duration_review_v1')
...
def run():
    ...
    summary={'status':'post-campaign internal numerical review','settings':'K48/T0.9 -> K76/T1.2 with paired inputs and enough history for both',
    ...
    save_json('results/room/joint_refinement.json',summary);print(json.dumps(summary,indent=2),flush=True)
